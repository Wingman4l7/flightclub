...
class Node implements CameraSubject {
    ...
    */
    ...
    /**
       ...
       already awake we still wake up the triggers. Why?
	   An *overlapping* node may have sent one of the triggers to sleep.
     */
    void wakeUp(float t) {
		for (int i = 0; i < next; i++) {
			triggers[i].wakeUp(t);
    ...
    /**
       Adds a visual representation of this node to the model. 
	   We draw a big circle on the ground showing the coverage of the node.
     */
