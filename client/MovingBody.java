...
abstract class MovingBody extends Particle {
	...
	turning). We set the roll rate such that after turning through
	one (1) radian the roll is at its max value.  
    */
    private void updateRoll(float dt) {

		rollRate = (speed/turnRadius) * rollMax;

		// special case if speed is zero - 
		// glider landed but still has to roll level!
		if (speed == 0) {
    ...
    public void tick(float t, float dt) {
		updateRoll(dt); // roll		
		super.tick(t, dt); // p and v
		obj.setFrame(p, v, roll); // pass message to obj
    }
