...
public class Obj3d implements CameraSubject {
	...
	throws IOException, FileFormatException {
		...
		for (int i = 0; i < num; i++) {
			...
			try {
				...
				throw new FileFormatException("Unable to parse color: " + st.sval + 
											  ", polygon index: " + i);
			}
    ...
    */
    ...
    /**
       Adds a point to the list. First scan through the list 
	   to see if we already have a point with identical coords. 
	   If so, return index of that point.
	*/
    ...
    /**
       Adds a polygon. Note that the vertices of the polygon 
       are passed using a float[][] and NOT a float[]. Eg. 
       
         {{x0, y0, z0}, {x1, y1, z1}, ...}  

       We *flatten* the data once it is encapsulated inside this class;
       outside this class we want clarity; inside this class we want speed!
    */
    ...
    */
    ...
    /**
       ...
       We are passed four points that do NOT lie in a plane.
    ...
    public void lathePolygons(float[][] ps, Color[] color, int steps) {
		...
		for (int step = 0; step < steps; step++) {
			...
			for (int point = 0; point < ps.length - 1; point++) {
				this.addPolygon(new float[][] {_ps[point], ps[point], ps[point + 1], 
											   _ps[point + 1]}, color[currColor]);
			}
    ...
    */
    ...
    public static Obj3d makeCube(ModelViewer modelViewer) {
		...
		/*
			Two steps. First we create a cube whose corner is at the origin
			and then we translate it so that its center is at the origin.
		*/
    ...
    /** 
	Loops thru' surfaces setting their dirtyNormal flags
	so that the surface normals will be computed again.
    ...
    */
    ...
    /**
       Adds a polygon. And then add a second polygon - 
       the shadow - using the same points with z ~ 0
    */
    ...
    /**
       Keeps the shadow under its object. The owner/creator of this 
       object should call this method each time they move the object.
    */
    ...
    */
    class Polygon {
		...
		*/
		...
		/**
		   Draws this polygon on the screen. If any of the points are not visible
		   (ie. behind the camera) then do not attempt to draw this polygon.  
		*/
    ...
    /**
       This inner class represents a wire. 
	   The wire is made from N points (or vertices).
    */
    class Polywire {
		int n; //number of points eg. 4 for a square
		int[] points; //list of indexes for the points that make up this Polygon
		...
		/**
		   Draws this polywire on the screen. If a point is not visible
		   (ie. behind the camera) then do not attempt to draw the line 
    ...
    /**
       This class implements a bounding box in model
       space that contains the parent Obj3d instance.  
    */
    class BB {
		float xmin, xmax, ymin, ymax, zmin, zmax;
		public BB() {;}

		/** Loop through all points and find the min and max for each dimension. */
		void setBB() {
			for (int i = 0; i < npoints * 3; i += 3) {
				if (i == 0) {
					xmin = xmax = ps[0];
